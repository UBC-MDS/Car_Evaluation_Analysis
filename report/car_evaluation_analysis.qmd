...
The SVM RBF model also showed exceptional ability in determining the acceptability of cars as seen in accuracy. With an f1 score of 0.99 on test data. Because we see that there is a class imbalance on the dateset, we also calculate the F1 score
...
After performing hyperparameter optimisation, the SVM RBF model manage to achieve the best score of `{python} test_scores_df['f1_score'].values[0]` on the test.
