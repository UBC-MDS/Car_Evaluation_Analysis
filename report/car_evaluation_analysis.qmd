...
We use a data created by the efforts of M. Bohanec in the late 1980s. It is sourced from the UCI Machine Learning Repository and is publicly available for research. Every feature and target in this dataset is encoded by one-hot encoder for data analysis.
...
The Car Evaluation Dataset was created as part of efforts to understand the factors that affect the acceptability of cars among consumers. These factors include buying price of a car, maintenance costs, passenger and luggage capacity, and safety. "Acceptability" is categoriezed as a categorical variable with four levels; "Unacceptable", "Acceptable", "Good", and "Very Good." A consumer's decision to purchase a car, and the price a consumer is willing to pay, is influenced by the condition of the car. The goal of this project is to develop a machine learning model that can evaluate the quality of a car based on its attributes to help buyers make a more informed decision for their next car purchase. Due to many factors determining the condition of a car, this is not an easy decision for consumers to make on their own.
