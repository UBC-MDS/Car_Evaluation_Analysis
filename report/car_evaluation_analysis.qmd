...
model_selection_results = pd.read_csv("../results/tables/model_selection_results.csv")
...
According to the results, SVM RBF achieved high train and cross-validation scores of `{python} round(model_selection_results['Mean train score'][3], 3)` and `{python} round(model_selection_results['Mean CV score'][3], 3)` respectively, 
...
As the score report has indicated, our model has performed extremely well, achieving a test accuracy of 0.991. This means that our model is predicting well on unseen data.

## Results & Discussion

After performing hyperparameter optimisation, the best SVM RBF model managed to achieve a score of 0.99 on the test set. This suggests the model has been generalised well, with high scores on both the train and test sets.
