...
To assess the balance of target classes in the dataset, the distribution of the target variable was visualized (see @fig-targetdist). We can see the dataset has a significant class imbalance which is the `unacceptable` class. This imbalance could impact the performance of classification models and may require further handling. For example, oversampling, undersampling, or class-weighted loss functions during training.
...
After performing hyperparameter optimisation, the SVM RBF model manage to achieve the best f1 score of `{python} test_scores_df['f1_score'].values[0]` on the test.
